    ...
    setupShaders() {
        ...
        const fragmentShaderSource = `
            ...
            float scanline(vec2 uv) {
                return sin(uv.y * u_resolution.y * 1.5) * 0.04;
            }

            // Film grain
            float grain(vec2 uv) {
                return random(uv * u_time) * 0.05;
            }
            ...
            void main() {
                ...
                // VHS color shift on edges
                float edge = abs(uv.y - 0.5) * 2.0;
                vec3 color = vec3(0.0);
                
                if (edge > 0.8) {
                    color = vec3(0.1, 0.0, 0.1); // Purple tint at edges
                }
